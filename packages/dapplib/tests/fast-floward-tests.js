...
describe("Flow Dapp Tests", async () => {
  ...
  describe("Fast Floward Tests", async () => {
    ...
    it(`removes a kittyitem from sale`, async () => {
      ...
      let numItems = await DappLib.kittyItemsMarketReadSaleCollectionLength(
        testData2
      );
      // 6) call kittyItemsMarketReadSaleCollectionIDs with testData3 and store the result
      let IDarr = await DappLib.kittyItemsMarketReadSaleCollectionIDs(
        testData3
      );
      ...
      assert.equal(
        numItems.result,
        1,
        "User should have 1 KittyItem up for sale. nor true"
      );
    ...
    it(`checks both admin and user have correct balances after purchase`, async () => {
      ...
      let bal0 = await DappLib.kibbleGetBalance(testData1);
      // 4) call kibbleGetBalance with testData2 and store the result
      let bal1 = await DappLib.kibbleGetBalance(testData2);
      // 5) assert the result from step 3) equals 5.0
      assert.equal(bal0.result, 5.0, "Admin balance not 5.0");
      // 6) assert the result from step 4) equals 25.0
      assert.equal(bal1.result, 25.0, "User balance not 25.0");
    });
  });
});
